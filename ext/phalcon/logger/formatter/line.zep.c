...
PHP_METHOD(Phalcon_Logger_Formatter_Line, setDateFormat) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_dateFormat"), &dateFormat);
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Logger_Formatter_Line, setFormat) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_format"), &format);
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Logger_Formatter_Line, format) {
	...
	zephir_read_property(&format, this_ptr, ZEND_STRL("_format"), PH_NOISY_CC);
	if (zephir_memnstr_str(&format, SL("%date%"), "phalcon/logger/formatter/line.zep", 80)) {
		ZEPHIR_INIT_VAR(&_0$$3);
		zephir_read_property(&_1$$3, this_ptr, ZEND_STRL("_dateFormat"), PH_NOISY_CC | PH_READONLY);
		ZVAL_LONG(&_2$$3, timestamp);
		ZEPHIR_CALL_FUNCTION(&_3$$3, "date", NULL, 238, &_1$$3, &_2$$3);
