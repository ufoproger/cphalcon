...
PHP_METHOD(Phalcon_Logger_Adapter_Syslog, __construct) {
	...
	if (zephir_is_true(name)) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "openlog", NULL, 234, name, &option, &facility);
		zephir_check_call_status();
		if (1) {
			zephir_update_property_zval(this_ptr, ZEND_STRL("_opened"), &__$true);
		} else {
			zephir_update_property_zval(this_ptr, ZEND_STRL("_opened"), &__$false);
		}
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Logger_Adapter_Syslog, logInternal) {
	...
	ZEPHIR_CALL_FUNCTION(NULL, "syslog", NULL, 235, &_3, &_4);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Logger_Adapter_Syslog, close) {
	...
	zephir_read_property(&_0, this_ptr, ZEND_STRL("_opened"), PH_NOISY_CC | PH_READONLY);
	if (!(zephir_is_true(&_0))) {
		RETURN_MM_BOOL(1);
	}
	ZEPHIR_RETURN_CALL_FUNCTION("closelog", NULL, 236);
	zephir_check_call_status();
	RETURN_MM();
