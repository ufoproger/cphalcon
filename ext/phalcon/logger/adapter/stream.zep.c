...
PHP_METHOD(Phalcon_Logger_Adapter_Stream, __construct) {
	...
	ZEPHIR_CALL_FUNCTION(&stream, "fopen", NULL, 229, &name, &mode);
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_stream"), &stream);
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Logger_Adapter_Stream, logInternal) {
	...
	zephir_read_property(&stream, this_ptr, ZEND_STRL("_stream"), PH_NOISY_CC);
...
PHP_METHOD(Phalcon_Logger_Adapter_Stream, close) {
	...
	zephir_read_property(&_0, this_ptr, ZEND_STRL("_stream"), PH_NOISY_CC | PH_READONLY);
