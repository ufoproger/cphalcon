...
PHP_METHOD(Phalcon_Session_Adapter_Redis, __construct) {
	...
	if (zephir_array_isset_string_fetch(&lifetime, &options, SL("lifetime"), 0)) {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_lifetime"), &lifetime);
	}
	ZEPHIR_INIT_VAR(&_2);
	...
	ZEPHIR_CALL_METHOD(NULL, &_2, "__construct", NULL, 273, &_3, &options);
	zephir_check_call_status();
	...
	ZEPHIR_CALL_FUNCTION(NULL, "session_set_save_handler", NULL, 0, &_6, &_8, &_9, &_10, &_11, &_12);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Session_Adapter_Redis, destroy) {
	...
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_0);
	zephir_read_property(&_1, this_ptr, ZEND_STRL("_redis"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CALL_METHOD(&_2, &_1, "exists", NULL, 0, &id);
	zephir_check_call_status();
	if (zephir_is_true(&_2)) {
		zephir_read_property(&_3, this_ptr, ZEND_STRL("_redis"), PH_NOISY_CC | PH_READONLY);
