...
PHP_METHOD(Phalcon_Annotations_Annotation, __construct) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_name"), &_0);
	...
	if (zephir_array_isset_string_fetch(&exprArguments, &reflectionData, SL("arguments"), 0)) {
		...
		ZEPHIR_INIT_NVAR(&argument);
		zephir_update_property_zval(this_ptr, ZEND_STRL("_arguments"), &arguments);
		zephir_update_property_zval(this_ptr, ZEND_STRL("_exprArguments"), &exprArguments);
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Annotations_Annotation, getExpression) {
	...
	do {
		...
		if (ZEPHIR_IS_LONG(&type, 308)) {
			...
			if (Z_TYPE_P(&_0$$7) == IS_ARRAY) {
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_0$$7), _1$$7)
				{
					...
					ZEPHIR_CALL_METHOD(&resolvedItem, this_ptr, "getexpression", &_4, 78, &_3$$8);
					zephir_check_call_status();
					ZEPHIR_OBS_NVAR(&name);
				...
				while (1) {
					...
					zephir_check_call_status();
						...
						ZEPHIR_CALL_METHOD(&resolvedItem, this_ptr, "getexpression", &_4, 78, &_5$$11);
						zephir_check_call_status();
						ZEPHIR_OBS_NVAR(&name);
						if (zephir_array_isset_string_fetch(&name, &item, SL("name"), 0)) {
							zephir_array_update_zval(&arrayValue, &name, &resolvedItem, PH_COPY | PH_SEPARATE);
		...
		if (ZEPHIR_IS_LONG(&type, 300)) {
			...
			ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 79, &expr);
			zephir_check_call_status();
			RETURN_MM();
		}
