...
PHP_METHOD(Phalcon_Events_Manager, attach) {
	...
	zephir_read_property(&_0, this_ptr, ZEND_STRL("_events"), PH_NOISY_CC | PH_READONLY);
	if (!(zephir_array_isset_fetch(&priorityQueue, &_0, &eventType, 0))) {
		zephir_read_property(&_1$$4, this_ptr, ZEND_STRL("_enablePriorities"), PH_NOISY_CC | PH_READONLY);
		...
		if (zephir_is_true(&_1$$4)) {
			...
			ZVAL_LONG(&_2$$5, 1);
			ZEPHIR_CALL_METHOD(NULL, &priorityQueue, "setextractflags", NULL, 176, &_2$$5);
			zephir_check_call_status();
	...
	if (Z_TYPE_P(&priorityQueue) == IS_OBJECT) {
		...
		ZEPHIR_CALL_METHOD(NULL, &priorityQueue, "insert", NULL, 177, handler, &_3$$7);
		zephir_check_call_status();
	} else {
...
PHP_METHOD(Phalcon_Events_Manager, detach) {
	...
	if (zephir_array_isset_fetch(&priorityQueue, &_0, &eventType, 0)) {
		if (Z_TYPE_P(&priorityQueue) == IS_OBJECT) {
			...
			ZVAL_LONG(&_1$$5, 1);
			ZEPHIR_CALL_METHOD(NULL, &newPriorityQueue, "setextractflags", NULL, 176, &_1$$5);
			zephir_check_call_status();
			ZVAL_LONG(&_1$$5, 3);
			...
			while (1) {
				...
				if (!ZEPHIR_IS_IDENTICAL(&_5$$6, handler)) {
					...
					ZEPHIR_CALL_METHOD(NULL, &newPriorityQueue, "insert", &_8, 177, &_6$$7, &_7$$7);
					zephir_check_call_status();
				}
			}
			zephir_update_property_array(this_ptr, SL("_events"), &eventType, &newPriorityQueue);
		} else {
			ZEPHIR_CALL_FUNCTION(&key, "array_search", NULL, 178, handler, &priorityQueue, &__$true);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Events_Manager, enablePriorities) {
	...
	if (enablePriorities) {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_enablePriorities"), &__$true);
	} else {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_enablePriorities"), &__$false);
	}
...
PHP_METHOD(Phalcon_Events_Manager, collectResponses) {
	...
	if (collect) {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_collect"), &__$true);
	} else {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_collect"), &__$false);
	}
...
PHP_METHOD(Phalcon_Events_Manager, detachAll) {
	...
	if (ZEPHIR_IS_STRING_IDENTICAL(&type, "")) {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_events"), &__$null);
	} else {
		zephir_read_property(&_0$$4, this_ptr, ZEND_STRL("_events"), PH_NOISY_CC | PH_READONLY);
		if (zephir_array_isset(&_0$$4, &type)) {
			zephir_read_property(&_1$$5, this_ptr, ZEND_STRL("_events"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Events_Manager, fireQueue) {
	...
	if (Z_TYPE_P(queue) != IS_ARRAY) {
		if (Z_TYPE_P(queue) == IS_OBJECT) {
			if (!(zephir_instance_of_ev(queue, spl_ce_SplPriorityQueue))) {
				...
				ZEPHIR_CALL_FUNCTION(&_3$$5, "sprintf", NULL, 138, &_2$$5, &_1$$5);
				zephir_check_call_status();
	...
	ZEPHIR_OBS_VAR(&_5);
	zephir_read_property(&_5, this_ptr, ZEND_STRL("_collect"), PH_NOISY_CC);
...
PHP_METHOD(Phalcon_Events_Manager, fire) {
	...
	zephir_read_property(&events, this_ptr, ZEND_STRL("_events"), PH_NOISY_CC);
	...
	ZEPHIR_INIT_VAR(&status);
	ZVAL_NULL(&status);
	zephir_read_property(&_2, this_ptr, ZEND_STRL("_collect"), PH_NOISY_CC | PH_READONLY);
	if (zephir_is_true(&_2)) {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_responses"), &__$null);
	}
	...
	if (zephir_array_isset_fetch(&fireEvents, &events, &type, 0)) {
		...
		if (_3$$6) {
			...
			ZEPHIR_CALL_METHOD(NULL, &event, "__construct", NULL, 179, &eventName, source, data, &_4$$7);
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&status, this_ptr, "firequeue", NULL, 180, &fireEvents, &event);
			zephir_check_call_status();
		}
	}
	...
	if (zephir_array_isset_fetch(&fireEvents, &events, &eventType, 0)) {
		...
		if (_5$$8) {
			if (Z_TYPE_P(&event) == IS_NULL) {
				...
				ZEPHIR_CALL_METHOD(NULL, &event, "__construct", NULL, 179, &eventName, source, data, &_6$$10);
				zephir_check_call_status();
			}
			ZEPHIR_CALL_METHOD(&status, this_ptr, "firequeue", NULL, 180, &fireEvents, &event);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Events_Manager, getListeners) {
	...
	zephir_read_property(&events, this_ptr, ZEND_STRL("_events"), PH_NOISY_CC | PH_READONLY);
	if (Z_TYPE_P(&events) == IS_ARRAY) {
