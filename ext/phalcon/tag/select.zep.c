...
PHP_METHOD(Phalcon_Tag_Select, selectField) {
	...
	if (Z_TYPE_P(&options) == IS_OBJECT) {
		...
		ZEPHIR_CALL_SELF(&_8$$24, "_optionsfromresultset", &_9, 370, &options, &using, &value, &_11$$24);
		zephir_check_call_status();
		...
		if (Z_TYPE_P(&options) == IS_ARRAY) {
			...
			ZEPHIR_CALL_SELF(&_12$$26, "_optionsfromarray", &_13, 371, &options, &value, &_15$$26);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Tag_Select, _optionsFromArray) {
	...
	if (Z_TYPE_P(data) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(data), _2, _3, _0)
		{
			...
			ZEPHIR_CALL_FUNCTION(&escaped, "htmlspecialchars", &_4, 173, &optionValue);
			zephir_check_call_status();
			if (Z_TYPE_P(&optionText) == IS_ARRAY) {
				ZEPHIR_INIT_NVAR(&_5$$4);
				ZEPHIR_GET_CONSTANT(&_5$$4, "PHP_EOL");
				ZEPHIR_CALL_SELF(&_6$$4, "_optionsfromarray", &_7, 371, &optionText, value, closeOption);
				zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (Z_TYPE_P(&optionText) == IS_ARRAY) {
					...
					ZEPHIR_CALL_SELF(&_17$$12, "_optionsfromarray", &_7, 371, &optionText, value, closeOption);
					zephir_check_call_status();
