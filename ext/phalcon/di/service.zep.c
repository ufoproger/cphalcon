...
PHP_METHOD(Phalcon_Di_Service, __construct) {
	...
	if (shared) {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_shared"), &__$true);
	} else {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_shared"), &__$false);
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Di_Service, setSharedInstance) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_sharedInstance"), sharedInstance);
...
PHP_METHOD(Phalcon_Di_Service, resolve) {
	...
	zephir_read_property(&shared, this_ptr, ZEND_STRL("_shared"), PH_NOISY_CC);
	if (zephir_is_true(&shared)) {
		ZEPHIR_OBS_VAR(&sharedInstance);
		zephir_read_property(&sharedInstance, this_ptr, ZEND_STRL("_sharedInstance"), PH_NOISY_CC);
	...
	if (Z_TYPE_P(&definition) == IS_STRING) {
		...
		if (Z_TYPE_P(&definition) == IS_OBJECT) {
			...
			if (Z_TYPE_P(&definition) == IS_ARRAY) {
				...
				ZEPHIR_CALL_METHOD(&instance, &builder, "build", NULL, 167, dependencyInjector, &definition, parameters);
				zephir_check_call_status();
			} else {
				found = 0;
			}
	...
	if (found == 0) {
		...
		zephir_read_property(&_3$$22, this_ptr, ZEND_STRL("_name"), PH_NOISY_CC | PH_READONLY);
	...
	if (zephir_is_true(&shared)) {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_sharedInstance"), &instance);
	}
	if (1) {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_resolved"), &__$true);
	} else {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_resolved"), &__$false);
	}
...
PHP_METHOD(Phalcon_Di_Service, __set_state) {
	...
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 56, &name, &definition, &shared);
	zephir_check_call_status();
	RETURN_MM();
