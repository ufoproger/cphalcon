...
PHP_METHOD(Phalcon_Application, registerModules) {
	...
	if (merge) {
		...
		zephir_read_property(&_1$$3, this_ptr, ZEND_STRL("_modules"), PH_NOISY_CC | PH_READONLY);
		zephir_fast_array_merge(&_0$$3, &_1$$3, &modules);
		zephir_update_property_zval(this_ptr, ZEND_STRL("_modules"), &_0$$3);
	} else {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_modules"), &modules);
	}
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Application, setDefaultModule) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_defaultModule"), &defaultModule);
	RETURN_THIS();
...
zend_object *zephir_init_properties_Phalcon_Application(zend_class_entry *class_type) {
	...
	
	{
		...
		zephir_read_property_ex(&_0, this_ptr, ZEND_STRL("_modules"), PH_NOISY_CC | PH_READONLY);
		if (Z_TYPE_P(&_0) == IS_NULL) {
			ZEPHIR_INIT_VAR(&_1$$3);
			array_init(&_1$$3);
			zephir_update_property_zval_ex(this_ptr, ZEND_STRL("_modules"), &_1$$3);
		}
		ZEPHIR_MM_RESTORE();
