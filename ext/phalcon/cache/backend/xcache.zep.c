...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, get) {
	...
	ZEPHIR_CALL_FUNCTION(&cachedContent, "xcache_get", NULL, 76, &prefixedKey);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, save) {
	...
	ZEPHIR_CALL_FUNCTION(&success, "xcache_set", NULL, 77, &lastKey, &preparedContent, &tt1);
	zephir_check_call_status();
	...
	if (zephir_is_true(&success)) {
		...
		zephir_read_property(&_1$$17, this_ptr, ZEND_STRL("_options"), PH_NOISY_CC | PH_READONLY);
		...
		if (!ZEPHIR_IS_STRING(&specialKey, "")) {
			ZEPHIR_CALL_FUNCTION(&keys, "xcache_get", NULL, 76, &specialKey);
			zephir_check_call_status();
			...
			ZEPHIR_CALL_FUNCTION(NULL, "xcache_set", NULL, 77, &specialKey, &keys);
			zephir_check_call_status();
		}
	}
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, delete) {
	...
	ZEPHIR_CONCAT_SVV(&prefixedKey, "_PHCX", &_0, keyName);
	ZEPHIR_OBS_VAR(&specialKey);
	zephir_read_property(&_1, this_ptr, ZEND_STRL("_options"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, exists) {
	...
	if (zephir_is_true(&lastKey)) {
		ZEPHIR_RETURN_CALL_FUNCTION("xcache_isset", NULL, 104, &lastKey);
		zephir_check_call_status();
		RETURN_MM();
	}
	RETURN_MM_BOOL(0);
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, increment) {
	...
	if ((zephir_function_exists_ex(ZEND_STRL("xcache_inc")) == SUCCESS)) {
		...
		ZEPHIR_CALL_FUNCTION(&newVal, "xcache_inc", NULL, 105, &lastKey, &_1$$6);
		zephir_check_call_status();
	} else {
		ZEPHIR_CALL_FUNCTION(&origVal, "xcache_get", NULL, 76, &lastKey);
		zephir_check_call_status();
		ZEPHIR_INIT_NVAR(&newVal);
		ZVAL_LONG(&newVal, (zephir_get_numberval(&origVal) - value));
		ZEPHIR_CALL_FUNCTION(NULL, "xcache_set", NULL, 77, &lastKey, &newVal);
		zephir_check_call_status();
	}
	RETURN_CCTOR(&newVal);
...
PHP_METHOD(Phalcon_Cache_Backend_Xcache, decrement) {
	...
	if ((zephir_function_exists_ex(ZEND_STRL("xcache_dec")) == SUCCESS)) {
		ZVAL_LONG(&_1$$6, value);
		ZEPHIR_CALL_FUNCTION(&newVal, "xcache_dec", NULL, 106, &lastKey, &_1$$6);
		zephir_check_call_status();
	} else {
		ZEPHIR_CALL_FUNCTION(&origVal, "xcache_get", NULL, 76, &lastKey);
		zephir_check_call_status();
		ZEPHIR_INIT_NVAR(&newVal);
		ZVAL_LONG(&newVal, (zephir_get_numberval(&origVal) - value));
		ZEPHIR_CALL_FUNCTION(&success, "xcache_set", NULL, 77, &lastKey, &newVal);
		zephir_check_call_status();
	}
	RETURN_CCTOR(&newVal);
	...
	if (Z_TYPE_P(&keys) == IS_ARRAY) {
		...
		if (Z_TYPE_P(&keys) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&keys), _4$$5, _5$$5, _2$$5)
			{
				...
				ZEPHIR_CALL_FUNCTION(NULL, "xcache_unset", &_6, 107, &key);
				zephir_check_call_status();
			} ZEND_HASH_FOREACH_END();
		} else {
			ZEPHIR_CALL_METHOD(NULL, &keys, "rewind", NULL, 0);
