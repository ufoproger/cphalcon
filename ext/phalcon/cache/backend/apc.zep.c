...
PHP_METHOD(Phalcon_Cache_Backend_Apc, get) {
	...
	ZEPHIR_CALL_FUNCTION(&cachedContent, "apc_fetch", NULL, 74, &prefixedKey);
	zephir_check_call_status();
	if (ZEPHIR_IS_FALSE_IDENTICAL(&cachedContent)) {
		RETURN_MM_NULL();
	}
	zephir_read_property(&_1, this_ptr, ZEND_STRL("_frontend"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Cache_Backend_Apc, save) {
	...
	ZEPHIR_CALL_FUNCTION(&success, "apc_store", NULL, 75, &lastKey, &preparedContent, &ttl);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cache_Backend_Apc, increment) {
	...
	if ((zephir_function_exists_ex(ZEND_STRL("apc_inc")) == SUCCESS)) {
		...
		ZEPHIR_CALL_FUNCTION(&result, "apc_inc", NULL, 95, &prefixedKey, &_1$$3);
		zephir_check_call_status();
		RETURN_CCTOR(&result);
	} else {
		ZEPHIR_CALL_FUNCTION(&cachedContent, "apc_fetch", NULL, 74, &prefixedKey);
...
PHP_METHOD(Phalcon_Cache_Backend_Apc, decrement) {
	...
	if ((zephir_function_exists_ex(ZEND_STRL("apc_dec")) == SUCCESS)) {
		ZVAL_LONG(&_1$$3, value);
		ZEPHIR_RETURN_CALL_FUNCTION("apc_dec", NULL, 96, &lastKey, &_1$$3);
		zephir_check_call_status();
		RETURN_MM();
	} else {
		ZEPHIR_CALL_FUNCTION(&cachedContent, "apc_fetch", NULL, 74, &lastKey);
		zephir_check_call_status();
		if (zephir_is_numeric(&cachedContent)) {
...
PHP_METHOD(Phalcon_Cache_Backend_Apc, delete) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("apc_delete", NULL, 97, &_1);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Cache_Backend_Apc, exists) {
	...
	if (zephir_is_true(&lastKey)) {
		ZEPHIR_CALL_FUNCTION(&_1$$5, "apc_exists", NULL, 98, &lastKey);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cache_Backend_Apc, flush) {
	...
	for (;_1->funcs->valid(_1) == SUCCESS && !EG(exception); _1->funcs->move_forward(_1)) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "apc_delete", &_5, 97, &_4$$3);
		zephir_check_call_status();
	}
