...
PHP_METHOD(Phalcon_Cache_Backend_Apcu, exists) {
	...
	if (Z_TYPE_P(keyName) == IS_NULL) {
		ZEPHIR_OBS_VAR(&_0$$3);
		zephir_read_property(&_0$$3, this_ptr, ZEND_STRL("_lastKey"), PH_NOISY_CC);
		zephir_get_strval(&_1$$3, &_0$$3);
		ZEPHIR_CPY_WRT(&lastKey, &_1$$3);
	} else {
		zephir_read_property(&_2$$4, this_ptr, ZEND_STRL("_prefix"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Cache_Backend_Apcu, flush) {
	...
	for (;_5->funcs->valid(_5) == SUCCESS && !EG(exception); _5->funcs->move_forward(_5)) {
		...
		zephir_array_fetch_string(&_6$$6, &item, SL("key"), PH_NOISY | PH_READONLY, "phalcon/cache/backend/apcu.zep", 283);
