...
PHP_METHOD(Phalcon_Cache_Backend_File, __construct) {
	...
	if (zephir_array_isset_string_fetch(&safekey, &options, SL("safekey"), 0)) {
		...
		zephir_update_property_zval(this_ptr, ZEND_STRL("_useSafeKey"), &safekey);
	}
	ZEPHIR_OBS_VAR(&prefix);
	if (zephir_array_isset_string_fetch(&prefix, &options, SL("prefix"), 0)) {
		zephir_read_property(&_0$$6, this_ptr, ZEND_STRL("_useSafeKey"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Cache_Backend_File, get) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_lastKey"), &prefixedKey);
	ZEPHIR_OBS_VAR(&cacheDir);
	zephir_read_property(&_2, this_ptr, ZEND_STRL("_options"), PH_NOISY_CC | PH_READONLY);
	...
	if ((zephir_file_exists(&cacheFile) == SUCCESS) == 1) {
		...
		if (!(lifetime)) {
			...
			zephir_read_property(&lastLifetime, this_ptr, ZEND_STRL("_lastLifetime"), PH_NOISY_CC);
		...
		ZEPHIR_CALL_FUNCTION(NULL, "clearstatcache", NULL, 99, &__$true, &cacheFile);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cache_Backend_File, delete) {
	...
	if ((zephir_file_exists(&cacheFile) == SUCCESS)) {
		ZEPHIR_RETURN_CALL_FUNCTION("unlink", NULL, 100, &cacheFile);
		zephir_check_call_status();
		RETURN_MM();
	}
	RETURN_MM_BOOL(0);
...
PHP_METHOD(Phalcon_Cache_Backend_File, queryKeys) {
	...
	ZEPHIR_CALL_METHOD(NULL, &_4, "__construct", NULL, 101, &cacheDir);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cache_Backend_File, exists) {
	...
	if (zephir_is_true(&lastKey)) {
		zephir_read_property(&_1$$5, this_ptr, ZEND_STRL("_options"), PH_NOISY_CC | PH_READONLY);
		...
		if ((zephir_file_exists(&cacheFile) == SUCCESS)) {
			if (!(lifetime)) {
				zephir_read_property(&_3$$7, this_ptr, ZEND_STRL("_frontend"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Cache_Backend_File, increment) {
	...
	if ((zephir_file_exists(&cacheFile) == SUCCESS)) {
		...
		zephir_read_property(&lifetime, this_ptr, ZEND_STRL("_lastLifetime"), PH_NOISY_CC);
...
PHP_METHOD(Phalcon_Cache_Backend_File, decrement) {
	...
	if ((zephir_file_exists(&cacheFile) == SUCCESS)) {
		...
		zephir_read_property(&lifetime, this_ptr, ZEND_STRL("_lastLifetime"), PH_NOISY_CC);
		if (!(zephir_is_true(&lifetime))) {
			zephir_read_property(&_4$$4, this_ptr, ZEND_STRL("_frontend"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Cache_Backend_File, flush) {
	...
	ZEPHIR_CALL_METHOD(NULL, &_2, "__construct", NULL, 101, &cacheDir);
	zephir_check_call_status();
	...
	for (;_1->funcs->valid(_1) == SUCCESS && !EG(exception); _1->funcs->move_forward(_1)) {
		...
		if (EXPECTED(ZEPHIR_IS_TRUE(&_3$$4))) {
			...
			if (_4$$5) {
				ZEPHIR_CALL_FUNCTION(&_5$$6, "unlink", &_6, 100, &cacheFile);
				zephir_check_call_status();
...
PHP_METHOD(Phalcon_Cache_Backend_File, getKey) {
	...
	zephir_read_property(&_0, this_ptr, ZEND_STRL("_useSafeKey"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Cache_Backend_File, useSafeKey) {
	...
	if (useSafeKey) {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_useSafeKey"), &__$true);
	} else {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_useSafeKey"), &__$false);
	}
	RETURN_THISW();
