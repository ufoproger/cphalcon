...
PHP_METHOD(Phalcon_Cache_Backend_Libmemcached, _connect) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_memcache"), &memcache);
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Cache_Backend_Libmemcached, get) {
	...
	zephir_read_property(&memcache, this_ptr, ZEND_STRL("_memcache"), PH_NOISY_CC);
	...
	if (zephir_is_numeric(&cachedContent)) {
		...
		zephir_read_property(&_1$$6, this_ptr, ZEND_STRL("_frontend"), PH_NOISY_CC | PH_READONLY);
