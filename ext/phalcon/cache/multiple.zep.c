...
PHP_METHOD(Phalcon_Cache_Multiple, __construct) {
	...
	if (Z_TYPE_P(backends) != IS_NULL) {
		...
		zephir_update_property_zval(this_ptr, ZEND_STRL("_backends"), backends);
	}
...
PHP_METHOD(Phalcon_Cache_Multiple, push) {
	...
	ZEND_PARSE_PARAMETERS_START(1, 1)
		Z_PARAM_OBJECT_OF_CLASS(backend, phalcon_cache_backendinterface_ce)
	ZEND_PARSE_PARAMETERS_END();
...
PHP_METHOD(Phalcon_Cache_Multiple, get) {
	...
	zephir_read_property(&_0, this_ptr, ZEND_STRL("_backends"), PH_NOISY_CC | PH_READONLY);
