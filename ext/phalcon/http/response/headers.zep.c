...
PHP_METHOD(Phalcon_Http_Response_Headers, get) {
	...
	zephir_read_property(&headers, this_ptr, ZEND_STRL("_headers"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Http_Response_Headers, remove) {
	...
	zephir_read_property(&headers, this_ptr, ZEND_STRL("_headers"), PH_NOISY_CC);
...
PHP_METHOD(Phalcon_Http_Response_Headers, send) {
	...
	if (!(zephir_is_true(&_0))) {
		zephir_read_property(&_1$$3, this_ptr, ZEND_STRL("_headers"), PH_NOISY_CC | PH_READONLY);
		...
		if (Z_TYPE_P(&_1$$3) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_1$$3), _4$$3, _5$$3, _2$$3)
			{
				...
				if (Z_TYPE_P(&value) != IS_NULL) {
					...
					ZEPHIR_CALL_FUNCTION(NULL, "header", &_7, 214, &_6$$5, &__$true);
					zephir_check_call_status();
				} else {
					...
					if (_8$$6) {
						ZEPHIR_CALL_FUNCTION(NULL, "header", &_7, 214, &header, &__$true);
						zephir_check_call_status();
					} else {
						ZEPHIR_INIT_NVAR(&_12$$8);
						ZEPHIR_CONCAT_VS(&_12$$8, &header, ": ");
						ZEPHIR_CALL_FUNCTION(NULL, "header", &_7, 214, &_12$$8, &__$true);
						zephir_check_call_status();
					}
				}
			...
			while (1) {
				...
				zephir_check_call_status();
					if (Z_TYPE_P(&value) != IS_NULL) {
						...
						ZEPHIR_CALL_FUNCTION(NULL, "header", &_7, 214, &_13$$10, &__$true);
						zephir_check_call_status();
					} else {
						...
						if (_14$$11) {
							...
							ZEPHIR_CALL_FUNCTION(NULL, "header", &_7, 214, &_18$$13, &__$true);
							zephir_check_call_status();
						}
					}
	...
	if (zephir_array_isset_string_fetch(&dataHeaders, &data, SL("_headers"), 0)) {
		...
		if (Z_TYPE_P(&dataHeaders) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&dataHeaders), _2$$3, _3$$3, _0$$3)
			{
				...
				ZEPHIR_CALL_METHOD(NULL, &headers, "set", &_4, 215, &key, &value);
				zephir_check_call_status();
			} ZEND_HASH_FOREACH_END();
		} else {
