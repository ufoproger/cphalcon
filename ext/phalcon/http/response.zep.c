...
PHP_METHOD(Phalcon_Http_Response, __construct) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_headers"), &_0);
		...
		if (Z_TYPE_P(&currentHeadersRaw) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&currentHeadersRaw), _3$$3, _4$$3, _1$$3)
			{
				...
				if (_5$$4) {
					...
					ZEPHIR_CALL_FUNCTION(&_7$$4, "strstr", &_8, 212, &key, &_6$$4);
					zephir_check_call_status();
			...
			while (1) {
				...
				zephir_check_call_status();
					...
					if (_10$$6) {
						...
						ZEPHIR_CALL_FUNCTION(&_12$$6, "strstr", &_8, 212, &key, &_11$$6);
						zephir_check_call_status();
...
PHP_METHOD(Phalcon_Http_Response, setHeaders) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_headers"), headers);
	RETURN_THISW();
...
PHP_METHOD(Phalcon_Http_Response, setCookies) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_cookies"), cookies);
	RETURN_THISW();
...
PHP_METHOD(Phalcon_Http_Response, redirect) {
	...
	if (externalRedirect) {
		...
		if (_0$$5) {
			...
			ZVAL_STRING(&_1$$5, "://");
			ZEPHIR_CALL_FUNCTION(&_2$$5, "strstr", NULL, 212, location, &_1$$5);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Http_Response, appendContent) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_content"), &_1);
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Http_Response, sendCookies) {
	...
	zephir_read_property(&cookies, this_ptr, ZEND_STRL("_cookies"), PH_NOISY_CC);
...
PHP_METHOD(Phalcon_Http_Response, send) {
	...
	zephir_read_property(&_0, this_ptr, ZEND_STRL("_sent"), PH_NOISY_CC | PH_READONLY);
	...
	zephir_read_property(&content, this_ptr, ZEND_STRL("_content"), PH_NOISY_CC);
	if (Z_TYPE_P(&content) != IS_NULL) {
		zend_print_zval(&content, 0);
	} else {
		ZEPHIR_OBS_VAR(&file);
		zephir_read_property(&file, this_ptr, ZEND_STRL("_file"), PH_NOISY_CC);
	...
	if (1) {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_sent"), &__$true);
	} else {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_sent"), &__$false);
	}
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Http_Response, setFileToSend) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_file"), &filePath);
	RETURN_THIS();
