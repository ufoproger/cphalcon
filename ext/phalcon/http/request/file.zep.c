...
PHP_METHOD(Phalcon_Http_Request_File, __construct) {
	...
	if (zephir_array_isset_string_fetch(&name, &file, SL("name"), 0)) {
		...
		ZEPHIR_CALL_FUNCTION(&_1$$3, "defined", NULL, 24, &_0$$3);
		zephir_check_call_status();
		if (zephir_is_true(&_1$$3)) {
			ZVAL_LONG(&_2$$4, 4);
			ZEPHIR_CALL_FUNCTION(&_3$$4, "pathinfo", NULL, 64, &name, &_2$$4);
			zephir_check_call_status();
			zephir_update_property_zval(this_ptr, ZEND_STRL("_extension"), &_3$$4);
		}
	}
	if (zephir_array_isset_string_fetch(&tempName, &file, SL("tmp_name"), 1)) {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_tmp"), &tempName);
	}
	...
	if (zephir_array_isset_string_fetch(&error, &file, SL("error"), 1)) {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_error"), &error);
	}
...
PHP_METHOD(Phalcon_Http_Request_File, getRealType) {
	...
	ZEPHIR_CALL_FUNCTION(&finfo, "finfo_open", NULL, 0, &_0);
	zephir_check_call_status();
	if (Z_TYPE_P(&finfo) != IS_RESOURCE) {
		RETURN_MM_STRING("");
	}
	zephir_read_property(&_1, this_ptr, ZEND_STRL("_tmp"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CALL_FUNCTION(&mime, "finfo_file", NULL, 0, &finfo, &_1);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Http_Request_File, isUploadedFile) {
	...
	if (_0) {
		ZEPHIR_CALL_FUNCTION(&_1, "is_uploaded_file", NULL, 210, &tmp);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Http_Request_File, moveTo) {
	...
	zephir_read_property(&_0, this_ptr, ZEND_STRL("_tmp"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_RETURN_CALL_FUNCTION("move_uploaded_file", NULL, 211, &_0, &destination);
	zephir_check_call_status();
	RETURN_MM();
