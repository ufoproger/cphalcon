...
PHP_METHOD(Phalcon_Http_Cookie, __construct) {
	...
	ZVAL_LONG(&_0, expire);
	zephir_update_property_zval(this_ptr, ZEND_STRL("_expire"), &_0);
	if (!ZEPHIR_IS_STRING_IDENTICAL(&path, "")) {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_path"), &path);
	}
	if (secure != 0) {
		if (secure) {
			zephir_update_property_zval(this_ptr, ZEND_STRL("_secure"), &__$true);
		} else {
			zephir_update_property_zval(this_ptr, ZEND_STRL("_secure"), &__$false);
		}
	}
	if (!ZEPHIR_IS_STRING_IDENTICAL(&domain, "")) {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_domain"), &domain);
	}
	if (httpOnly != 0) {
		if (httpOnly) {
			zephir_update_property_zval(this_ptr, ZEND_STRL("_httpOnly"), &__$true);
		} else {
			zephir_update_property_zval(this_ptr, ZEND_STRL("_httpOnly"), &__$false);
		}
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Http_Cookie, setValue) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_value"), value);
	if (1) {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_readed"), &__$true);
	} else {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_readed"), &__$false);
	}
	RETURN_THISW();
...
PHP_METHOD(Phalcon_Http_Cookie, getValue) {
	...
	zephir_read_property(&_0, this_ptr, ZEND_STRL("_restored"), PH_NOISY_CC | PH_READONLY);
	...
	ZEPHIR_OBS_VAR(&name);
	zephir_read_property(&name, this_ptr, ZEND_STRL("_name"), PH_NOISY_CC);
	zephir_read_property(&_1, this_ptr, ZEND_STRL("_readed"), PH_NOISY_CC | PH_READONLY);
	if (ZEPHIR_IS_FALSE_IDENTICAL(&_1)) {
		ZEPHIR_OBS_VAR(&value);
		if (zephir_array_isset_fetch(&value, &_COOKIE, &name, 0)) {
			zephir_read_property(&_2$$5, this_ptr, ZEND_STRL("_useEncryption"), PH_NOISY_CC | PH_READONLY);
			if (zephir_is_true(&_2$$5)) {
				zephir_read_property(&_3$$6, this_ptr, ZEND_STRL("_dependencyInjector"), PH_NOISY_CC | PH_READONLY);
			...
			zephir_update_property_zval(this_ptr, ZEND_STRL("_value"), &decryptedValue);
			if (Z_TYPE_P(filters) != IS_NULL) {
				...
				if (Z_TYPE_P(&filter) != IS_OBJECT) {
					if (Z_TYPE_P(&dependencyInjector) == IS_NULL) {
						zephir_read_property(&_6$$14, this_ptr, ZEND_STRL("_dependencyInjector"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Http_Cookie, send) {
	...
	zephir_read_property(&name, this_ptr, ZEND_STRL("_name"), PH_NOISY_CC);
	ZEPHIR_OBS_VAR(&value);
	zephir_read_property(&value, this_ptr, ZEND_STRL("_value"), PH_NOISY_CC);
	ZEPHIR_OBS_VAR(&expire);
	zephir_read_property(&expire, this_ptr, ZEND_STRL("_expire"), PH_NOISY_CC);
	ZEPHIR_OBS_VAR(&domain);
	zephir_read_property(&domain, this_ptr, ZEND_STRL("_domain"), PH_NOISY_CC);
	ZEPHIR_OBS_VAR(&path);
	zephir_read_property(&path, this_ptr, ZEND_STRL("_path"), PH_NOISY_CC);
	ZEPHIR_OBS_VAR(&secure);
	zephir_read_property(&secure, this_ptr, ZEND_STRL("_secure"), PH_NOISY_CC);
	ZEPHIR_OBS_VAR(&httpOnly);
	zephir_read_property(&httpOnly, this_ptr, ZEND_STRL("_httpOnly"), PH_NOISY_CC);
	...
	zephir_read_property(&_3, this_ptr, ZEND_STRL("_useEncryption"), PH_NOISY_CC | PH_READONLY);
	if (zephir_is_true(&_3)) {
		if (!(ZEPHIR_IS_EMPTY(&value))) {
			...
			zephir_read_property(&_6$$12, this_ptr, ZEND_STRL("signKey"), PH_NOISY_CC | PH_READONLY);
	...
	if (!(zephir_is_true(&_0))) {
		...
		if (Z_TYPE_P(&dependencyInjector) == IS_OBJECT) {
			...
			if (zephir_is_true(&_2$$4)) {
				zephir_read_property(&_3$$5, this_ptr, ZEND_STRL("_name"), PH_NOISY_CC | PH_READONLY);
				...
				if (Z_TYPE_P(&definition) == IS_ARRAY) {
					if (zephir_array_isset_string_fetch(&expire, &definition, SL("expire"), 1)) {
						zephir_update_property_zval(this_ptr, ZEND_STRL("_expire"), &expire);
					}
					...
					if (zephir_array_isset_string_fetch(&secure, &definition, SL("secure"), 1)) {
						zephir_update_property_zval(this_ptr, ZEND_STRL("_secure"), &secure);
					}
					if (zephir_array_isset_string_fetch(&httpOnly, &definition, SL("httpOnly"), 1)) {
						zephir_update_property_zval(this_ptr, ZEND_STRL("_httpOnly"), &httpOnly);
					}
				}
			}
		}
		if (1) {
			zephir_update_property_zval(this_ptr, ZEND_STRL("_restored"), &__$true);
		} else {
			zephir_update_property_zval(this_ptr, ZEND_STRL("_restored"), &__$false);
		}
	}
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Http_Cookie, delete) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_value"), &__$null);
	ZEPHIR_INIT_VAR(&_4);
	zephir_time(&_4);
	ZVAL_LONG(&_0, (zephir_get_numberval(&_4) - 691200));
	ZEPHIR_CALL_FUNCTION(NULL, "setcookie", NULL, 192, &name, &__$null, &_0, &path, &domain, &secure, &httpOnly);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Http_Cookie, useEncryption) {
	...
	if (useEncryption) {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_useEncryption"), &__$true);
	} else {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_useEncryption"), &__$false);
	}
	RETURN_THISW();
...
PHP_METHOD(Phalcon_Http_Cookie, setExpiration) {
	...
	ZVAL_LONG(&_1, expire);
	zephir_update_property_zval(this_ptr, ZEND_STRL("_expire"), &_1);
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Http_Cookie, assertSignKeyIsLongEnough) {
	...
	ZEPHIR_CALL_FUNCTION(&length, "mb_strlen", NULL, 193, &signKey);
	zephir_check_call_status();
	if (ZEPHIR_LT_LONG(&length, 32)) {
		...
		ZEPHIR_CALL_FUNCTION(&_2$$3, "sprintf", NULL, 138, &_1$$3, &length);
		zephir_check_call_status();
