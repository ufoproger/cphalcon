...
PHP_METHOD(Phalcon_Forms_Element, __construct) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_messages"), &_1);
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Forms_Element, setForm) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_form"), form);
	RETURN_THISW();
...
PHP_METHOD(Phalcon_Forms_Element, addFilter) {
	...
	if (Z_TYPE_P(&filters) == IS_ARRAY) {
		...
		if (Z_TYPE_P(&filters) == IS_STRING) {
			...
			zephir_update_property_zval(this_ptr, ZEND_STRL("_filters"), &_0$$5);
		} else {
			ZEPHIR_INIT_VAR(&_1$$6);
			zephir_create_array(&_1$$6, 1, 0);
			zephir_array_fast_append(&_1$$6, &filter);
			zephir_update_property_zval(this_ptr, ZEND_STRL("_filters"), &_1$$6);
		}
	}
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Forms_Element, addValidators) {
	...
	if (merge) {
		...
		zephir_read_property(&currentValidators, this_ptr, ZEND_STRL("_validators"), PH_NOISY_CC);
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_validators"), &mergedValidators);
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Forms_Element, prepareAttributes) {
	...
	zephir_read_property(&defaultAttributes, this_ptr, ZEND_STRL("_attributes"), PH_NOISY_CC);
...
PHP_METHOD(Phalcon_Forms_Element, getAttribute) {
	...
	zephir_read_property(&attributes, this_ptr, ZEND_STRL("_attributes"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Forms_Element, setLabel) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_label"), &label);
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Forms_Element, label) {
	...
	zephir_read_property(&label, this_ptr, ZEND_STRL("_label"), PH_NOISY_CC);
...
PHP_METHOD(Phalcon_Forms_Element, getValue) {
	...
	ZVAL_NULL(&value);
	ZEPHIR_OBS_VAR(&form);
	zephir_read_property(&form, this_ptr, ZEND_STRL("_form"), PH_NOISY_CC);
...
PHP_METHOD(Phalcon_Forms_Element, setMessages) {
	...
	bool is_null_true = 1;
	ZEND_PARSE_PARAMETERS_START(1, 1)
		Z_PARAM_OBJECT_OF_CLASS(group, phalcon_validation_message_group_ce)
	ZEND_PARSE_PARAMETERS_END();
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_messages"), group);
	RETURN_THISW();
...
PHP_METHOD(Phalcon_Forms_Element, clear) {
	...
	zephir_read_property(&_1, this_ptr, ZEND_STRL("_name"), PH_NOISY_CC | PH_READONLY);
	ZVAL_NULL(&_2);
...
zend_object *zephir_init_properties_Phalcon_Forms_Element(zend_class_entry *class_type) {
	...
	
	{
		...
		zephir_read_property_ex(&_0, this_ptr, ZEND_STRL("_validators"), PH_NOISY_CC | PH_READONLY);
		if (Z_TYPE_P(&_0) == IS_NULL) {
			ZEPHIR_INIT_VAR(&_1$$3);
			array_init(&_1$$3);
			zephir_update_property_zval_ex(this_ptr, ZEND_STRL("_validators"), &_1$$3);
		}
		ZEPHIR_MM_RESTORE();
