...
PHP_METHOD(Phalcon_Db_Result_Pdo, __construct) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_pdoStatement"), result);
...
PHP_METHOD(Phalcon_Db_Result_Pdo, execute) {
	...
	zephir_read_property(&_0, this_ptr, ZEND_STRL("_pdoStatement"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Db_Result_Pdo, fetchAll) {
	...
	zephir_read_property(&pdoStatement, this_ptr, ZEND_STRL("_pdoStatement"), PH_NOISY_CC);
...
PHP_METHOD(Phalcon_Db_Result_Pdo, numRows) {
	...
	zephir_read_property(&rowCount, this_ptr, ZEND_STRL("_rowCount"), PH_NOISY_CC);
	if (ZEPHIR_IS_FALSE_IDENTICAL(&rowCount)) {
		...
		if (ZEPHIR_IS_FALSE_IDENTICAL(&rowCount)) {
			...
			zephir_read_property(&sqlStatement, this_ptr, ZEND_STRL("_sqlStatement"), PH_NOISY_CC);
			if (!(zephir_start_with_str(&sqlStatement, SL("SELECT COUNT(*) ")))) {
				...
				if (zephir_is_true(&_2$$6)) {
					zephir_array_fetch_long(&_4$$7, &matches, 1, PH_NOISY | PH_READONLY, "phalcon/db/result/pdo.zep", 239);
					ZEPHIR_INIT_VAR(&_5$$7);
					ZEPHIR_CONCAT_SVS(&_5$$7, "SELECT COUNT(*) \"numrows\" FROM (SELECT ", &_4$$7, ")");
					zephir_read_property(&_6$$7, this_ptr, ZEND_STRL("_bindParams"), PH_NOISY_CC | PH_READONLY);
					zephir_read_property(&_7$$7, this_ptr, ZEND_STRL("_bindTypes"), PH_NOISY_CC | PH_READONLY);
					...
					zephir_array_fetch_string(&rowCount, &row, SL("numrows"), PH_NOISY, "phalcon/db/result/pdo.zep", 245);
				}
			} else {
		...
		zephir_update_property_zval(this_ptr, ZEND_STRL("_rowCount"), &rowCount);
	}
...
PHP_METHOD(Phalcon_Db_Result_Pdo, dataSeek) {
	...
	if (Z_TYPE_P(&bindParams) == IS_ARRAY) {
		...
		if (Z_TYPE_P(&statement) == IS_OBJECT) {
			zephir_read_property(&_1$$4, this_ptr, ZEND_STRL("_bindTypes"), PH_NOISY_CC | PH_READONLY);
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_pdoStatement"), &statement);
	n = -1;
	number--;
	while (1) {
		if (!(n != number)) {
...
PHP_METHOD(Phalcon_Db_Result_Pdo, setFetchMode) {
	...
	if (fetchMode == 8) {
		...
		if (zephir_is_true(&_0$$3)) {
			...
			zephir_update_property_zval(this_ptr, ZEND_STRL("_fetchMode"), &_2$$4);
			RETURN_MM_BOOL(1);
		}
		RETURN_MM_BOOL(0);
	}
	if (fetchMode == 9) {
		...
		if (zephir_is_true(&_3$$5)) {
			...
			zephir_update_property_zval(this_ptr, ZEND_STRL("_fetchMode"), &_5$$6);
			RETURN_MM_BOOL(1);
		}
		RETURN_MM_BOOL(0);
	}
	if (fetchMode == 7) {
		...
		if (zephir_is_true(&_6$$7)) {
			...
			zephir_update_property_zval(this_ptr, ZEND_STRL("_fetchMode"), &_8$$8);
			RETURN_MM_BOOL(1);
		}
		RETURN_MM_BOOL(0);
	}
	...
	if (zephir_is_true(&_9)) {
		...
		zephir_update_property_zval(this_ptr, ZEND_STRL("_fetchMode"), &_11$$9);
		RETURN_MM_BOOL(1);
	}
	RETURN_MM_BOOL(0);
