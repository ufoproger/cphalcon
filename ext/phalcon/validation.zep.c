...
PHP_METHOD(Phalcon_Validation, setValidators) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_validators"), validators);
	RETURN_THISW();
...
PHP_METHOD(Phalcon_Validation, __construct) {
	...
	ZEND_PARSE_PARAMETERS_START(0, 1)
		Z_PARAM_OPTIONAL
		Z_PARAM_ARRAY_OR_NULL(validators)
	ZEND_PARSE_PARAMETERS_END();
	...
	if (zephir_fast_count_int(&validators)) {
		...
		ZEPHIR_CALL_FUNCTION(&_1$$3, "array_filter", NULL, 381, &validators, &_0$$3);
		zephir_check_call_status();
		zephir_update_property_zval(this_ptr, ZEND_STRL("_validators"), &_1$$3);
		ZEPHIR_INIT_VAR(&_2$$3);
		ZEPHIR_INIT_NVAR(&_2$$3);
		zephir_create_closure_ex(&_2$$3, NULL, phalcon_2__closure_ce, SL("__invoke"));
		ZEPHIR_CALL_FUNCTION(&_3$$3, "array_filter", NULL, 381, &validators, &_2$$3);
		zephir_check_call_status();
		zephir_update_property_zval(this_ptr, ZEND_STRL("_combinedFieldsValidators"), &_3$$3);
	}
...
PHP_METHOD(Phalcon_Validation, validate) {
	...
	zephir_read_property(&validators, this_ptr, ZEND_STRL("_validators"), PH_NOISY_CC);
	ZEPHIR_OBS_VAR(&combinedFieldsValidators);
	zephir_read_property(&combinedFieldsValidators, this_ptr, ZEND_STRL("_combinedFieldsValidators"), PH_NOISY_CC);
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_values"), &__$null);
	...
	if ((zephir_method_exists_ex(this_ptr, ZEND_STRL("aftervalidation")) == SUCCESS)) {
		zephir_read_property(&_22$$34, this_ptr, ZEND_STRL("_messages"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Validation, rules) {
	...
	if (Z_TYPE_P(&validators) == IS_ARRAY) {
		...
		while (1) {
			...
			zephir_check_call_status();
				if (zephir_instance_of_ev(&validator, phalcon_validation_validatorinterface_ce)) {
					ZEPHIR_CALL_METHOD(NULL, this_ptr, "add", &_2, 187, field, &validator);
					zephir_check_call_status();
				}
			ZEPHIR_CALL_METHOD(NULL, &validators, "next", NULL, 0);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Validation, getFilters) {
	...
	zephir_read_property(&filters, this_ptr, ZEND_STRL("_filters"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Validation, setDefaultMessages) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_defaultMessages"), &_0);
...
PHP_METHOD(Phalcon_Validation, getDefaultMessage) {
	...
	zephir_read_property(&_0, this_ptr, ZEND_STRL("_defaultMessages"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Validation, setLabels) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_labels"), &labels);
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Validation, getLabel) {
	...
	zephir_read_property(&labels, this_ptr, ZEND_STRL("_labels"), PH_NOISY_CC);
	if (Z_TYPE_P(field) == IS_ARRAY) {
...
PHP_METHOD(Phalcon_Validation, appendMessage) {
	...
	ZEPHIR_CALL_METHOD(NULL, &messages, "appendmessage", NULL, 382, message);
	zephir_check_call_status();
	zephir_update_property_zval(this_ptr, ZEND_STRL("_messages"), &messages);
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Validation, getValue) {
	...
	if (Z_TYPE_P(&entity) == IS_OBJECT) {
		...
		zephir_read_property(&values, this_ptr, ZEND_STRL("_values"), PH_NOISY_CC);
			...
			if (Z_TYPE_P(&entity) == IS_OBJECT) {
				...
				if ((zephir_method_exists(&entity, &method)  == SUCCESS)) {
					...
					if ((zephir_method_exists_ex(&entity, ZEND_STRL("writeattribute")) == SUCCESS)) {
						...
						ZEPHIR_CALL_FUNCTION(&_4$$28, "property_exists", NULL, 262, &entity, &field);
						zephir_check_call_status();
	...
	if (Z_TYPE_P(field) == IS_ARRAY) {
		...
		if (Z_TYPE_P(field) == IS_ARRAY) {
			ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(field), _0$$3)
			{
				ZEPHIR_INIT_NVAR(&singleField);
				ZVAL_COPY(&singleField, _0$$3);
				ZEPHIR_CALL_METHOD(&result, this_ptr, "prechecking", &_2, 383, &singleField, validator);
				zephir_check_call_status();
		...
		if (zephir_is_true(&allowEmpty)) {
			...
			if (UNEXPECTED(_11$$9)) {
				...
				ZVAL_STRING(&_14$$18, "");
				zephir_array_fast_append(&_13$$18, &_14$$18);
				ZEPHIR_CALL_FUNCTION(&_15$$18, "in_array", NULL, 313, &raw$$9, &_13$$18, &__$true);
				zephir_check_call_status();
...
zend_object *zephir_init_properties_Phalcon_Validation(zend_class_entry *class_type) {
	...
	
	{
		...
		zephir_read_property_ex(&_0, this_ptr, ZEND_STRL("_labels"), PH_NOISY_CC | PH_READONLY);
		if (Z_TYPE_P(&_0) == IS_NULL) {
			ZEPHIR_INIT_VAR(&_1$$3);
			array_init(&_1$$3);
			zephir_update_property_zval_ex(this_ptr, ZEND_STRL("_labels"), &_1$$3);
		}
		...
		zephir_read_property_ex(&_4, this_ptr, ZEND_STRL("_combinedFieldsValidators"), PH_NOISY_CC | PH_READONLY);
		if (Z_TYPE_P(&_4) == IS_NULL) {
			ZEPHIR_INIT_VAR(&_5$$5);
			array_init(&_5$$5);
			zephir_update_property_zval_ex(this_ptr, ZEND_STRL("_combinedFieldsValidators"), &_5$$5);
		}
		zephir_read_property_ex(&_6, this_ptr, ZEND_STRL("_validators"), PH_NOISY_CC | PH_READONLY);
		if (Z_TYPE_P(&_6) == IS_NULL) {
			ZEPHIR_INIT_VAR(&_7$$6);
			array_init(&_7$$6);
			zephir_update_property_zval_ex(this_ptr, ZEND_STRL("_validators"), &_7$$6);
		}
		ZEPHIR_MM_RESTORE();
