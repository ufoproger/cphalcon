...
PHP_METHOD(Phalcon_Validation_Validator_StringLength, validate) {
	...
	if ((zephir_function_exists_ex(ZEND_STRL("mb_strlen")) == SUCCESS)) {
		ZEPHIR_CALL_FUNCTION(&length, "mb_strlen", NULL, 193, &value);
		zephir_check_call_status();
	} else {
	...
	if (zephir_is_true(&isSetMax)) {
		...
		if (ZEPHIR_GT(&length, &maximum)) {
			...
			ZEPHIR_CALL_METHOD(NULL, &_4$$8, "__construct", NULL, 384, &_6$$8, &field, &_5$$8, &code);
			zephir_check_call_status();
	...
	if (zephir_is_true(&isSetMin)) {
		...
		if (ZEPHIR_LT(&length, &minimum)) {
			...
			ZEPHIR_CALL_METHOD(NULL, &_9$$11, "__construct", NULL, 384, &_12$$11, &field, &_10$$11, &code);
			zephir_check_call_status();
