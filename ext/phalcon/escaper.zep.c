...
PHP_METHOD(Phalcon_Escaper, setEncoding) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_encoding"), &encoding);
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Escaper, setHtmlQuoteType) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_htmlQuoteType"), &_0);
...
PHP_METHOD(Phalcon_Escaper, setDoubleEncode) {
	...
	if (doubleEncode) {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_doubleEncode"), &__$true);
	} else {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_doubleEncode"), &__$false);
	}
...
PHP_METHOD(Phalcon_Escaper, detectEncoding) {
	...
	if (Z_TYPE_P(&_0) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_0), _2)
		{
			...
			ZEPHIR_CALL_FUNCTION(&_4$$5, "mb_detect_encoding", &_5, 170, &str, &charset, &__$true);
		...
		while (1) {
			...
			zephir_check_call_status();
				ZEPHIR_CALL_FUNCTION(&_6$$7, "mb_detect_encoding", &_5, 170, &str, &charset, &__$true);
				zephir_check_call_status();
	...
	ZEPHIR_INIT_NVAR(&charset);
	ZEPHIR_RETURN_CALL_FUNCTION("mb_detect_encoding", &_5, 170, &str);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Escaper, normalizeEncoding) {
	...
	ZEPHIR_CALL_METHOD(&_0, this_ptr, "detectencoding", NULL, 171, &str);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_1);
	ZVAL_STRING(&_1, "UTF-32");
	ZEPHIR_RETURN_CALL_FUNCTION("mb_convert_encoding", NULL, 172, &str, &_1, &_0);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Phalcon_Escaper, escapeHtml) {
	...
	zephir_read_property(&_0, this_ptr, ZEND_STRL("_htmlQuoteType"), PH_NOISY_CC | PH_READONLY);
	zephir_read_property(&_1, this_ptr, ZEND_STRL("_encoding"), PH_NOISY_CC | PH_READONLY);
	zephir_read_property(&_2, this_ptr, ZEND_STRL("_doubleEncode"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Escaper, escapeHtmlAttr) {
	...
	zephir_read_property(&_0, this_ptr, ZEND_STRL("_encoding"), PH_NOISY_CC | PH_READONLY);
	zephir_read_property(&_1, this_ptr, ZEND_STRL("_doubleEncode"), PH_NOISY_CC | PH_READONLY);
	ZVAL_LONG(&_2, 3);
...
PHP_METHOD(Phalcon_Escaper, escapeCss) {
	...
	ZEPHIR_CALL_METHOD(&_0, this_ptr, "normalizeencoding", NULL, 174, &css);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Escaper, escapeJs) {
	...
	ZEPHIR_CALL_METHOD(&_0, this_ptr, "normalizeencoding", NULL, 174, &js);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Escaper, escapeUrl) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("rawurlencode", NULL, 175, &url);
	zephir_check_call_status();
	RETURN_MM();
