...
PHP_METHOD(Phalcon_Mvc_Application, useImplicitView) {
	...
	if (implicitView) {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_implicitView"), &__$true);
	} else {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_implicitView"), &__$false);
	}
	RETURN_THISW();
...
PHP_METHOD(Phalcon_Mvc_Application, handle) {
	...
	zephir_read_property(&implicitView, this_ptr, ZEND_STRL("_implicitView"), PH_NOISY_CC);
	...
	zephir_read_property(&_0, this_ptr, ZEND_STRL("_sendHeaders"), PH_NOISY_CC | PH_READONLY);
	if (zephir_is_true(&_0)) {
		ZEPHIR_CALL_METHOD(NULL, &response, "sendheaders", NULL, 0);
		zephir_check_call_status();
	}
	zephir_read_property(&_41, this_ptr, ZEND_STRL("_sendCookies"), PH_NOISY_CC | PH_READONLY);
