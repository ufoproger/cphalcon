...
PHP_METHOD(Phalcon_Mvc_View_Simple, setViewsDir) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_viewsDir"), &viewsDir);
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Mvc_View_Simple, _loadTemplateEngines) {
	...
	if (ZEPHIR_IS_FALSE_IDENTICAL(&engines)) {
		...
		if (Z_TYPE_P(&registeredEngines) != IS_ARRAY) {
			...
			ZEPHIR_CALL_METHOD(NULL, &_0$$4, "__construct", NULL, 316, this_ptr, &dependencyInjector);
...
PHP_METHOD(Phalcon_Mvc_View_Simple, _internalRender) {
	...
	if (Z_TYPE_P(&eventsManager) == IS_OBJECT) {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_activeRenderPath"), &path);
	}
	if (Z_TYPE_P(&eventsManager) == IS_OBJECT) {
	...
	notExists = 1;
	mustClean = 1;
	zephir_read_property(&_2, this_ptr, ZEND_STRL("_viewsDir"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Mvc_View_Simple, render) {
	...
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "_internalrender", NULL, 335, &path, &mergedParams);
	zephir_check_call_status();
	if (Z_TYPE_P(&cache) == IS_OBJECT) {
...
PHP_METHOD(Phalcon_Mvc_View_Simple, partial) {
	...
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "_internalrender", NULL, 335, &partialPath, &mergedParams);
	zephir_check_call_status();
	if (Z_TYPE_P(params) == IS_ARRAY) {
...
PHP_METHOD(Phalcon_Mvc_View_Simple, setCacheOptions) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_cacheOptions"), &options);
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Mvc_View_Simple, cache) {
	...
	if (Z_TYPE_P(options) == IS_ARRAY) {
		if (1) {
			zephir_update_property_zval(this_ptr, ZEND_STRL("_cache"), &__$true);
		} else {
			zephir_update_property_zval(this_ptr, ZEND_STRL("_cache"), &__$false);
		}
		zephir_update_property_zval(this_ptr, ZEND_STRL("_cacheOptions"), options);
	} else {
...
PHP_METHOD(Phalcon_Mvc_View_Simple, setVars) {
	...
	if (_0) {
		ZEPHIR_OBS_VAR(&_1);
		zephir_read_property(&_1, this_ptr, ZEND_STRL("_viewParams"), PH_NOISY_CC);
		_0 = Z_TYPE_P(&_1) == IS_ARRAY;
	}
	if (_0) {
		ZEPHIR_INIT_VAR(&_2$$3);
		zephir_read_property(&_3$$3, this_ptr, ZEND_STRL("_viewParams"), PH_NOISY_CC | PH_READONLY);
		zephir_fast_array_merge(&_2$$3, &_3$$3, &params);
		zephir_update_property_zval(this_ptr, ZEND_STRL("_viewParams"), &_2$$3);
	} else {
