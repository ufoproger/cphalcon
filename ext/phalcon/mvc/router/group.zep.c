...
PHP_METHOD(Phalcon_Mvc_Router_Group, __construct) {
	...
	if (_0) {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_paths"), paths);
	}
...
PHP_METHOD(Phalcon_Mvc_Router_Group, beforeMatch) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_beforeMatch"), beforeMatch);
	RETURN_THISW();
...
PHP_METHOD(Phalcon_Mvc_Router_Group, _addRoute) {
	...
	zephir_read_property(&defaultPaths, this_ptr, ZEND_STRL("_paths"), PH_NOISY_CC);
	...
	ZEPHIR_CALL_METHOD(NULL, &route, "__construct", NULL, 67, &_2, &mergedPaths, httpMethods);
	zephir_check_call_status();
	zephir_update_property_array_append(this_ptr, SL("_routes"), &route);
	ZEPHIR_CALL_METHOD(NULL, &route, "setgroup", NULL, 314, this_ptr);
	zephir_check_call_status();
	RETURN_CCTOR(&route);
