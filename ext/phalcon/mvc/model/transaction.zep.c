...
PHP_METHOD(Phalcon_Mvc_Model_Transaction, begin) {
	...
	zephir_read_property(&_0, this_ptr, ZEND_STRL("_connection"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Mvc_Model_Transaction, rollback) {
	...
	if (zephir_is_true(&_0)) {
		...
		if (Z_TYPE_P(rollbackRecord) == IS_OBJECT) {
			zephir_update_property_zval(this_ptr, ZEND_STRL("_rollbackRecord"), rollbackRecord);
		}
		ZEPHIR_INIT_VAR(&_1$$4);
		object_init_ex(&_1$$4, phalcon_mvc_model_transaction_failed_ce);
		zephir_read_property(&_2$$4, this_ptr, ZEND_STRL("_rollbackRecord"), PH_NOISY_CC | PH_READONLY);
		ZEPHIR_CALL_METHOD(NULL, &_1$$4, "__construct", NULL, 308, rollbackMessage, &_2$$4);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model_Transaction, getConnection) {
	...
	zephir_read_property(&_0, this_ptr, ZEND_STRL("_rollbackOnAbort"), PH_NOISY_CC | PH_READONLY);
	if (zephir_is_true(&_0)) {
		ZEPHIR_CALL_FUNCTION(&_1$$3, "connection_aborted", NULL, 309);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model_Transaction, setIsNewTransaction) {
	...
	if (isNew) {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_isNewTransaction"), &__$true);
	} else {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_isNewTransaction"), &__$false);
	}
...
PHP_METHOD(Phalcon_Mvc_Model_Transaction, setRollbackOnAbort) {
	...
	if (rollbackOnAbort) {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_rollbackOnAbort"), &__$true);
	} else {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_rollbackOnAbort"), &__$false);
	}
...
PHP_METHOD(Phalcon_Mvc_Model_Transaction, isManaged) {
	...
	ZEPHIR_OBS_VAR(&_0);
	zephir_read_property(&_0, this_ptr, ZEND_STRL("_manager"), PH_NOISY_CC);
...
PHP_METHOD(Phalcon_Mvc_Model_Transaction, setRollbackedRecord) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_rollbackRecord"), record);
