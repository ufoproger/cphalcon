...
PHP_METHOD(Phalcon_Mvc_Model_Transaction_Manager, setDbService) {
	...
	zephir_update_property_zval(this_ptr, ZEND_STRL("_service"), &service);
	RETURN_THIS();
...
PHP_METHOD(Phalcon_Mvc_Model_Transaction_Manager, setRollbackPendent) {
	...
	if (rollbackPendent) {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_rollbackPendent"), &__$true);
	} else {
		zephir_update_property_zval(this_ptr, ZEND_STRL("_rollbackPendent"), &__$false);
	}
	RETURN_THISW();
...
PHP_METHOD(Phalcon_Mvc_Model_Transaction_Manager, has) {
	...
	zephir_read_property(&_0, this_ptr, ZEND_STRL("_number"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Mvc_Model_Transaction_Manager, get) {
	...
	if (!(zephir_is_true(&_0))) {
		zephir_read_property(&_1$$3, this_ptr, ZEND_STRL("_rollbackPendent"), PH_NOISY_CC | PH_READONLY);
		if (zephir_is_true(&_1$$3)) {
			...
			ZEPHIR_CALL_FUNCTION(NULL, "register_shutdown_function", NULL, 310, &_2$$4);
			zephir_check_call_status();
		}
		if (1) {
...
PHP_METHOD(Phalcon_Mvc_Model_Transaction_Manager, getOrCreateTransaction) {
	...
	if (zephir_is_true(&_0)) {
		...
		zephir_read_property(&transactions, this_ptr, ZEND_STRL("_transactions"), PH_NOISY_CC);
	...
	zephir_read_property(&_5, this_ptr, ZEND_STRL("_service"), PH_NOISY_CC | PH_READONLY);
	if (autoBegin) {
		ZVAL_BOOL(&_6, 1);
	} else {
		ZVAL_BOOL(&_6, 0);
	}
	ZEPHIR_CALL_METHOD(NULL, &transaction, "__construct", NULL, 311, &dependencyInjector, &_6, &_5);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(NULL, &transaction, "settransactionmanager", NULL, 312, this_ptr);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model_Transaction_Manager, _collectTransaction) {
	...
	if (zephir_fast_count_int(&transactions)) {
		...
		ZEPHIR_INIT_NVAR(&managedTransaction);
		zephir_update_property_zval(this_ptr, ZEND_STRL("_transactions"), &newTransactions);
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Mvc_Model_Transaction_Manager, collectTransactions) {
	...
	if (zephir_fast_count_int(&transactions)) {
		...
		ZEPHIR_INIT_NVAR(&_0$$3);
		zephir_update_property_zval(this_ptr, ZEND_STRL("_transactions"), &__$null);
	}
	ZEPHIR_MM_RESTORE();
