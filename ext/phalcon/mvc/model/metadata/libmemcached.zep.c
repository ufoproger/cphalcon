...
PHP_METHOD(Phalcon_Mvc_Model_MetaData_Libmemcached, __construct) {
	...
	ZEPHIR_OBS_VAR(&_4);
	zephir_read_property(&_4, this_ptr, ZEND_STRL("_ttl"), PH_NOISY_CC);
	zephir_array_update_string(&_3, SL("lifetime"), &_4, PH_COPY | PH_SEPARATE);
	ZEPHIR_CALL_METHOD(NULL, &_2, "__construct", NULL, 270, &_3);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(NULL, &_1, "__construct", NULL, 271, &_2, options);
	zephir_check_call_status();
	zephir_update_property_zval(this_ptr, ZEND_STRL("_memcache"), &_1);
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Mvc_Model_MetaData_Libmemcached, read) {
	...
	zephir_read_property(&_0, this_ptr, ZEND_STRL("_memcache"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Phalcon_Mvc_Model_MetaData_Libmemcached, reset) {
	...
	ZEPHIR_OBS_VAR(&meta);
	zephir_read_property(&meta, this_ptr, ZEND_STRL("_metaData"), PH_NOISY_CC);
	if (Z_TYPE_P(&meta) == IS_ARRAY) {
		...
		if (Z_TYPE_P(&meta) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&meta), _3$$3, _4$$3, _1$$3)
			{
				...
				zephir_read_property(&_5$$4, this_ptr, ZEND_STRL("_memcache"), PH_NOISY_CC | PH_READONLY);
			...
			while (1) {
				...
				zephir_check_call_status();
					ZEPHIR_INIT_NVAR(&realKey);
					ZEPHIR_CONCAT_SV(&realKey, "meta-", &key);
					zephir_read_property(&_6$$5, this_ptr, ZEND_STRL("_memcache"), PH_NOISY_CC | PH_READONLY);
...
zend_object *zephir_init_properties_Phalcon_Mvc_Model_MetaData_Libmemcached(zend_class_entry *class_type) {
	...
	
	{
		...
		zephir_read_property_ex(&_0, this_ptr, ZEND_STRL("_metaData"), PH_NOISY_CC | PH_READONLY);
		if (Z_TYPE_P(&_0) == IS_NULL) {
			ZEPHIR_INIT_VAR(&_1$$3);
			array_init(&_1$$3);
			zephir_update_property_zval_ex(this_ptr, ZEND_STRL("_metaData"), &_1$$3);
		}
		ZEPHIR_MM_RESTORE();
