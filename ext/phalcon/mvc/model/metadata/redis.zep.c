...
PHP_METHOD(Phalcon_Mvc_Model_MetaData_Redis, __construct) {
	...
	ZEPHIR_OBS_VAR(&_7);
	zephir_read_property(&_7, this_ptr, ZEND_STRL("_ttl"), PH_NOISY_CC);
	zephir_array_update_string(&_6, SL("lifetime"), &_7, PH_COPY | PH_SEPARATE);
	ZEPHIR_CALL_METHOD(NULL, &_5, "__construct", NULL, 270, &_6);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(NULL, &_4, "__construct", NULL, 273, &_5, options);
	zephir_check_call_status();
	zephir_update_property_zval(this_ptr, ZEND_STRL("_redis"), &_4);
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Mvc_Model_MetaData_Redis, read) {
	...
	zephir_read_property(&_0, this_ptr, ZEND_STRL("_redis"), PH_NOISY_CC | PH_READONLY);
	...
	if (Z_TYPE_P(&meta) == IS_ARRAY) {
		...
		if (Z_TYPE_P(&meta) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&meta), _3$$3, _4$$3, _1$$3)
			{
				...
				zephir_read_property(&_5$$4, this_ptr, ZEND_STRL("_redis"), PH_NOISY_CC | PH_READONLY);
			...
			while (1) {
				...
				zephir_check_call_status();
					ZEPHIR_INIT_NVAR(&realKey);
					ZEPHIR_CONCAT_SV(&realKey, "meta-", &key);
					zephir_read_property(&_6$$5, this_ptr, ZEND_STRL("_redis"), PH_NOISY_CC | PH_READONLY);
...
zend_object *zephir_init_properties_Phalcon_Mvc_Model_MetaData_Redis(zend_class_entry *class_type) {
