...
PHP_METHOD(Phalcon_Mvc_Model_MetaData_Files, __construct) {
	...
	if (Z_TYPE_P(options) == IS_ARRAY) {
		if (zephir_array_isset_string_fetch(&metaDataDir, options, SL("metaDataDir"), 1)) {
			zephir_update_property_zval(this_ptr, ZEND_STRL("_metaDataDir"), &metaDataDir);
		}
	}
...
PHP_METHOD(Phalcon_Mvc_Model_MetaData_Files, read) {
	...
	zephir_read_property(&_0, this_ptr, ZEND_STRL("_metaDataDir"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_INIT_VAR(&_1);
	ZEPHIR_INIT_VAR(&_2);
	ZVAL_STRING(&_2, "_");
...
zend_object *zephir_init_properties_Phalcon_Mvc_Model_MetaData_Files(zend_class_entry *class_type) {
